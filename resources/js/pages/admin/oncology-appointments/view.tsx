...
export default function AppointmentsView({ appointments, filter: initialFilter, search: initialSearch, date_from: initialDateFrom, date_to: initialDateTo, sort: initialSort, direction: initialDirection, stats, routePrefix = '/admin/oncology-appointments', pageTitle: pageTitleProp }: AppointmentsViewProps) {
    const { t } = useTranslation();
    const pageTitle = pageTitleProp ?? t('oncology.pageTitle');
    ...
    const filterButtons = [
        { key: 'all', label: t('appointments.filterAll'), icon: Filter, count: stats.all },
                ...
                <div className="mx-auto flex max-w-7xl flex-col gap-5">
                    ...
                    <section className="card-gradient rounded-lg border border-white/50 p-5 shadow-sm shadow-[#2e3f84]/5 dark:border-white/10">
                        <div className="mb-4">
                            ...
                            <p className="text-sm settings-subtitle">{t('appointments.filtersDescription')}</p>
                        </div>
                        ...
                        <div className="relative">
                            <label htmlFor="view-search" className="sr-only">{t('oncology.searchAppointmentsLabel')}</label>
                    ...
                    <section className="card-gradient rounded-lg border border-white/50 p-5 shadow-sm shadow-[#2e3f84]/5 dark:border-white/10">
                        ...
                        {appointments.last_page > 1 && (
                            <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <p className="text-sm settings-subtitle">
                                    {t('oncology.pageOf', { current: appointments.current_page, total: appointments.last_page })}
                                </p>
