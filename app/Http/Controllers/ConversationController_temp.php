...
class ConversationController extends Controller
{
    ...
    public function show(Request $request, Conversation $conversation)
    {
        ...
        // Optimización: Cargar solo los últimos 50 mensajes para reducir payload inicial
        // Si hay más de 50 mensajes, el frontend podrá cargarlos bajo demanda
        $conversation->load(['messages' => function ($query) {
            $query->latest()->limit(50);
        }, 'messages.sender', 'assignedUser', 'resolvedByUser', 'tags']);

        // Reordenar mensajes cronológicamente (PHP side)
        // La relación 'messages' devuelve una Collection, podemos usar sortBy
        $conversation->setRelation('messages', $conversation->messages->sortBy('created_at')->values());

        // Verificar si hay más mensajes antiguos
        $oldestMessageId = $conversation->messages->first()?->id;
        $conversation->has_more_messages = $oldestMessageId 
